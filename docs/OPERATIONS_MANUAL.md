...
`0.4.0` 是本地持久化产品：SQLite 领域库、SQLite checkpointer 和本地内容寻址对象库组成
...
1. SQLite 领域事实库、active Registry、聚类运行、采样统计和知识图谱索引。
...
`0.1.1`/`0.1.2`/`0.2.0`/`0.2.1`/`0.3.0`/`0.3.1`/`0.4.0` 的领域 revision、Evidence 和 active Registry 设计为兼容；`0.2.0`
...
`0.4.0` 新增聚类、成员、图谱边、探索问题和采样统计表，均由 `initialize()` 非破坏性创建；
旧 Knowledge 缺少 `domain_hypotheses` 时会用现有 Scope 作为保守回填，不改写历史 revision。
未完成的旧
...
聚类运行还应监控：批次各采样桶数量、重复采样率、跨学科簇比例、非法 ID/关系过滤数、
语义边置信度、图谱扩展命中率、聚类调用延迟和 `auto_clustering` 失败事件。
...
- `UKA_CLUSTERING_ENABLED=true` 开启自动聚类；`UKA_CLUSTER_AUTO_EVERY` 控制未采样知识阈值，
  `UKA_CLUSTER_BATCH_SIZE` 控制批次大小（2..64），`UKA_LLM_TIMEOUT_SECONDS` 控制结构化模型调用
  上限（15..300 秒）。聚类失败不得回滚已激活知识，也不得静默伪造图谱边。
