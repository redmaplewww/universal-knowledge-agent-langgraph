...
入库 Scope 会保存最终领域和按概率排序的 `domain_hypotheses`。系统默认在累计 12 条尚未参与
聚类的新知识后，按 40% 领域覆盖、30% 信息缺失、20% 跨学科桥接和 10% 关键知识重采样组成
批次并调用 LLM。手工运行：
...
```python
result = agent.cluster_knowledge(
    tenant_id="demo",
    security_scope_id="private",
    batch_size=24,
)
clusters = agent.list_clusters(
    tenant_id="demo", security_scope_id="private"
)
...
HTTP 使用 `POST /v1/clustering/runs`、`GET /v1/clusters` 和
`GET /v1/knowledge-graph`。聚类边是检索索引假设，不是新事实；系统只沿高置信边扩展 FTS
结果，并重新执行原有 Scope、风险、冲突和 Evidence 完整性门禁。

Control Room 的“知识网络”使用力导向动态图展示当前 tenant/scope 的真实节点。节点大小随
直接连接数量变化，默认按聚类着色，也可切换为按节点类型着色。悬停节点会淡化无关内容并
突出一跳邻居；点击节点可固定焦点并在右侧查看领域概率、缺失信息和直接连接。画布支持节点
拖动、平移、滚轮缩放、适配视图和暂停漂移；搜索或类型筛选只保留匹配节点及其直接邻居。

相关配置：`UKA_CLUSTERING_ENABLED`、`UKA_CLUSTER_AUTO_EVERY`、
`UKA_CLUSTER_BATCH_SIZE`、`UKA_LLM_TIMEOUT_SECONDS`。

## 12. Python SDK 和 HTTP API
...
完整结构化聚类比健康探针耗时更长。先检查 `doctor --connect`，再根据模型延迟调整
`UKA_LLM_TIMEOUT_SECONDS`。不要把超时提高到无限；外部调用仍须有明确上限。模型鉴权失败应
修复受管配置，不能切换成离线 Provider 后宣称真实 LLM Gate 通过。
...
- [批次聚类与知识图谱报告](CLUSTERING_AND_KNOWLEDGE_GRAPH_REPORT_2026-09-19.md)
