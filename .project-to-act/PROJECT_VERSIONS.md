...
- 兼容性说明：Python 3.11+；LangGraph 1.2；与旧 `uka` 包无运行时兼容或导入关系；
  Experience v2、`approval_context`、Knowledge Gap、人工补证、领域候选和聚类索引均使用向后兼容
  字段；旧 Knowledge 可读取；旧 `knowledge_clusters` 表由 `initialize()` 非破坏性补充排序列；
  0.1.1 及更早的未完成 checkpoint 不自动迁移到安全 namespace。
- 最后更新：2026-09-20
...
- `0.4.0` maintenance｜2026-09-20｜移除旧外部测试框架、本地 Skill、脚本、缓存与历史测试产物；
  聚类 HTTP Gate 改为项目内 Python 标准库实现；真实 DeepSeek 功能 Gate 13/13、HTTP Gate 4/4、
  66 项回归通过｜不改变领域/API/SQLite 合同｜E-024｜G-011 通过。

- `0.4.0`｜2026-09-19｜批次聚类索引与跨学科知识图谱｜领域候选概率、覆盖/缺失/桥接/重采样
  四桶抽样、关键知识重复参与、真实 LLM 聚类、聚类/成员/语义边/探索持久化、缺口与潜在领域
  优先级、FTS 后高置信图谱扩展、CLI/SDK/HTTP、自动聚类与失败关闭｜SQLite 非破坏性增表/补列，
  Python 3.11+、LangGraph 1.2.10｜E-023｜G-010 通过；不等同生产图数据库或专家认证。
...
- `0.1.0`｜2026-08-03｜本地完整产品 Gate 通过｜Parser Registry、FTS EvidencePack、真实 GLM、SDK/API、完整治理、观测、wheel/clean-install E2E｜用户要求完整落地并使用 Skill 配置 LLM 测试｜Python 3.11+、LangGraph 1.2.10；与旧实验性工程隔离｜E-008..E-011｜G-002 通过；不宣称生产基础设施或专家质量认证。
- `0.1.0.dev0`｜2026-08-03｜P0 检查点通过｜独立 LangGraph Root/五子图、SQLite 事实库与 checkpoint、Evidence/Receipt、interrupt/resume、CLI、离线与可选模型 Provider｜用户明确要求独立实现｜不兼容且不依赖旧 实验性编排；Python 3.11+、LangGraph 1.2.10｜E-002..E-006｜G-001 通过；不代表 P1-P4 或生产就绪。
