...
- 当前阶段：`0.4.0` 批次聚类索引与跨学科知识图谱已完成
- 当前状态：A-001..A-023/G-010 已通过；本地隔离产品 Gate 完成，但不得宣称外部 IAM、生产容量或领域专家认证
- 最后更新：2026-09-19

## 项目目标

- 基于 LangGraph `StateGraph` 实现独立的知识 Agent，不依赖或导入旧 `uka` 项目及实验性编排。
...
- 证据不足时主动拒答，版本化沉淀 Knowledge Gap，有限 Web Search、后续证据精确回链与审批后关闭。
- 对准具体 Gap 手工提交证据，保持审批 Gate；中文来源的模型生成字段保持简体中文并对语言漂移失败关闭。
...
- 已交付里程碑：本地完整可运行 `0.3.1`；多格式、API、恢复、真实 LLM、文档级 Experience、
...
- 当前知识库已展示 Experience 的背景、问题、机制、行动、结果、依据、关系、边界、原文和演进谱系；
  中断审批会动态展示候选、风险、Scope、逻辑和原文，知识库默认以 5 条紧凑行呈现；证据不足的
  内容会拒答并进入 3 条默认上限的 Knowledge Gap 账本，可从词条内精确手工补证并进入审批；
  中文来源的生成字段保持中文，历史外文术语只作为原文或专名保留；下一决策点仍是
  生产外部存储、IAM、密钥托管、容量 SLO、真实多模态与领域专家质量 Gate。
- `0.4.0` 进一步保存领域候选概率，按覆盖/缺失/桥接/重采样四桶组成 LLM 聚类批次；聚类按
  信息缺口、潜在领域不确定性和跨域程度排序，关键知识可重复参与；tenant/scope 隔离图谱只沿
  高置信边辅助检索。真实 GLM 13/13、HTTP 4/4、65 项回归和洁净安装均通过。
...
- D-009｜2026-09-20｜移除旧外部测试框架及其本地 Skill、脚本、缓存和历史测试产物；后续 HTTP 验收由项目内 Python 标准库脚本负责｜用户明确要求彻底清除且该框架不再参与项目｜保留真实模型、HTTP 边界、租户隔离与失败关闭验证能力，不保留外部运行时依赖｜E-024｜用户明确指令｜后续测试工具变更时复审。
- D-008｜2026-09-19｜把领域概率、比例重采样、跨学科聚类与图谱辅助检索升级为一等知识层｜
  用户要求根据领域缺失程度与潜在领域概率排名、让关键知识重复聚类并参考 LLM-Skill 建立完整
  知识图谱｜新增 F-022、A-023、E-023/G-010；聚类和图谱边保持索引假设，Provider 超时失败关闭，
  不绕过 Evidence/Scope/Gap/审批｜真实 GLM 13/13、HTTP 验收 4/4、65 项回归、构建与洁净安装通过｜
  不豁免生产图数据库、容量或领域专家认证。

- D-007｜2026-08-11｜把补证从普通摄取升级为对准 Gap ID 的人工入口，并建立来源语言输出合同｜
  用户指出出现缺口后无法手工填补且中文对话出现英文缺口｜新增 F-021、A-022、E-022/G-009；
  人工证据仍必须审批，拒绝或证据不足不关闭；中文生成字段检测、受限二次修复和失败关闭；历史
  英文演示 Gap 通过追加 revision 本地化而不覆写谱系｜真实 `glm-5.2` HTTP 补证旅程、53 项回归、
  桌面/390px 浏览器和 0.3.1 洁净安装通过｜不豁免生产 IAM、来源信誉或专家认证。

- D-006｜2026-08-10｜增加认识论拒答、有限网络研究与 Knowledge Gap 补证闭环｜用户要求在思考和
  网络搜索后仍无结论时保留可能方向，并让后续知识注入正确链接｜新增 F-020、A-021、E-021/G-008；
  真实 GLM/Web Search/HTTP 三轮纠正驱动，最终四项核心比率均为 1.0；机密分类禁止外发｜代表性
  领域 Gate 不等同所有领域穷举或专家认证。

- D-005｜2026-08-10｜把审批中断从 ID 提示升级为完整决策单，并把知识库改为默认 5 条紧凑行｜
  用户指出审批步几乎没有信息且知识库过长｜新增 F-019、A-020、E-020/G-007；`approval_context`
  在授权响应时动态解析，不把原文写入 checkpoint；API 向后兼容｜真实 GLM 高风险候选、桌面/390px
  浏览器、47 项回归与 0.2.1 洁净安装｜后续生产审批仍需外部 IAM 和审计策略。
- D-004｜2026-08-10｜把沉淀单位从行级 Claim 升级为文档级 Experience，并让 active Knowledge
  ...
  伪造 v2 字段｜真实 GLM、HTTP 验收 115 条 Ledger、4 领域和浏览器验收｜后续生产演进仍不得绕过 Gate。
...
- D-001｜2026-08-03｜新建同级独立工程 `universal-knowledge-agent-langgraph`，旧目录仅作架构文档来源｜旧工程的包、测试和依赖直接耦合外部编排与测试工具，就地叠加会违反用户“不掺杂、独立 Agent 项目”的明确要求｜新工程拥有独立包名、依赖、状态、测试和账本｜E-001｜用户明确指令｜P0 隔离与安装态 Gate 通过后复审。
