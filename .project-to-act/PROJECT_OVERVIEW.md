...
- 当前阶段：`0.1.2` 深层完整性与隔离修复已完成
- 当前状态：A-001..A-018/G-005 已通过；本地隔离产品 Gate 完成，但不得宣称外部 IAM、生产容量或领域专家认证
- 最后更新：2026-08-04
...
- 已交付里程碑：本地完整可运行 `0.1.2`；多格式、API、恢复、真实 LLM、任意领域、深层完整性与洁净安装 Gate 均通过。
- 已关闭质量问题：自由领域标签、混合文档 Claim–Scope 绑定、`review_required` 检索失败关闭和源标识检索已由 F-016 修复并经 G-004 验证。
- 下一决策点：如需生产部署，须另行确定外部存储、IAM、密钥托管、容量 SLO、真实多模态与领域专家质量 Gate。
