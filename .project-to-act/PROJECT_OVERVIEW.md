...
- `0.4.0` 进一步保存领域候选概率，按覆盖/缺失/桥接/重采样四桶组成 LLM 聚类批次；聚类按
  ...
  高置信边辅助检索。真实 GLM 13/13、AAWO HTTP 4/4、65 项回归和洁净安装均通过。
...
- D-008｜2026-09-19｜把领域概率、比例重采样、跨学科聚类与图谱辅助检索升级为一等知识层｜
  ...
  不绕过 Evidence/Scope/Gap/审批｜真实 GLM 13/13、AAWO 4/4、65 项回归、构建与洁净安装通过｜
  不豁免生产图数据库、容量或领域专家认证。
...
- D-006｜2026-08-10｜增加认识论拒答、有限网络研究与 Knowledge Gap 补证闭环｜用户要求在思考和
  ...
  真实 GLM/Web Search/AAWO 三轮纠正驱动，最终四项核心比率均为 1.0；机密分类禁止外发｜代表性
