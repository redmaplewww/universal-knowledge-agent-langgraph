...
- 入库 Scope 同时保存最终领域和按概率排序的潜在领域候选；系统按领域覆盖、信息缺失、跨学科
  桥接和关键知识重采样四种比例拉取批次，由 LLM 生成聚类索引、跨知识关系和探索问题。
- 聚类成员、领域、Knowledge Gap 和高置信语义边组成 tenant/scope 隔离的知识图谱；FTS 首轮召回
  可沿高置信图谱边扩展，但仍须通过 Scope、风险、冲突和 Evidence 完整性门禁。
...
```powershell
uv run uka-lg --project-root . cluster `
  --tenant demo `
  --scope private `
  --batch-size 24
```

`Settings.load` 默认在累计 12 条未聚类的新知识后自动执行一轮聚类。可通过
`UKA_CLUSTERING_ENABLED`、`UKA_CLUSTER_AUTO_EVERY`、`UKA_CLUSTER_BATCH_SIZE` 和
`UKA_LLM_TIMEOUT_SECONDS` 调整。
...
uka-lg cluster --tenant <id> --scope <id> [--batch-size 24]
...
| `POST` | `/v1/clustering/runs` | 按比例抽样并运行一轮真实 LLM 聚类 |
| `GET` | `/v1/clusters` | 展示聚类、领域概率、缺失信息和成员 |
| `GET` | `/v1/knowledge-graph` | 展示 Knowledge、Domain、Cluster、Gap 与关系边 |
...
- 聚类和图谱边是带 Provider revision、run ID、置信度和支持知识 ID 的索引推断，不自动成为
  新的 Knowledge、事实或 Skill；非法 ID、自环和未授权关系在 Provider 合同层被过滤。
...
固定局部子图，同时保留搜索、筛选、缩放、拖动、暂停漂移和窄屏布局。

跨领域批量验证可复用 `scripts/cross_domain_dataset_20260917.py`、
`scripts/run_cross_domain_ingest_batch.py` 和 `scripts/run_cross_domain_retrieval_eval.py`；脚本只通过
真实 HTTP 边界读取本地受管模型配置，不包含 API Key。
...
- [人工补证与中文一致性修复报告](docs/MANUAL_GAP_SUPPLEMENT_AND_LANGUAGE_FIX_2026-08-11.md)。
- [批次聚类与知识图谱报告](docs/CLUSTERING_AND_KNOWLEDGE_GRAPH_REPORT_2026-09-19.md)。
- [面向非技术读者的推广资料](docs/PROMOTION.md)。

## 版本与授权说明

当前版本是 `0.4.0`。`pyproject.toml` 使用 `LicenseRef-Proprietary`；本仓库可公开查看，
