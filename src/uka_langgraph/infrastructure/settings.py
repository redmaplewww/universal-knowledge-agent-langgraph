...
class Settings:
    ...
    cluster_auto_every: int = 0
    cluster_batch_size: int = 24
    ...
    def load(cls, project_root: Path | str | None = None) -> Settings:
        ...
        clustering_value = os.getenv("UKA_CLUSTERING_ENABLED", "true").strip().lower()
        clustering_enabled = clustering_value in {"1", "true", "yes", "enabled"}
        return cls(
            project_root=root,
            state_dir=state_dir,
            ...
            cluster_auto_every=max(
                0, min(int(os.getenv("UKA_CLUSTER_AUTO_EVERY", "12")), 1000)
            ),
            cluster_batch_size=max(
                2, min(int(os.getenv("UKA_CLUSTER_BATCH_SIZE", "24")), 64)
            ),
            llm_timeout_seconds=max(
                15.0, min(float(os.getenv("UKA_LLM_TIMEOUT_SECONDS", "120")), 300.0)
            ),
        )
    ...
    def safe_status(self) -> dict[str, object]:
        return {
            ...
            "cluster_auto_every": self.cluster_auto_every,
