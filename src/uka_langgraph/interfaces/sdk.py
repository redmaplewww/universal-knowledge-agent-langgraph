...
class UniversalKnowledgeAgent:
                ...
                entries.append(
                    {
                        ...
                        "domain_hypotheses": list(
                            scope_payload.get("domain_hypotheses", [])
                        ),
    ...
    def list_clusters(
        ...
        *,
        ...
        with AgentRuntime(self.settings) as runtime:
            security = SecurityScope(tenant_id, security_scope_id)
            return _public(
                runtime.services.repository.list_clusters(security, limit)
            )
    ...
    def knowledge_graph(
        ...
        *,
        ...
        with AgentRuntime(self.settings) as runtime:
            security = SecurityScope(tenant_id, security_scope_id)
            snapshot = runtime.services.repository.graph_snapshot(security, limit)
            knowledge = runtime.services.repository.list_active_knowledge(
                security, min(limit, 1000)
            )
            gaps = runtime.services.repository.list_open_gaps(
                security, min(limit, 1000)
            )
            ...
            for item in knowledge:
                scope_id = str(item.payload.get("scope_id", ""))
                scope = (
                    runtime.services.repository.get_revision(
                ...
                nodes.append(
                    {
                        ...
                        "summary": _bounded_text(item.payload.get("content", ""), 500),
                ...
                for domain_id in domain_ids:
                    ...
                    edges.append(
                        {
                            "edge_id": f"domain:{item.object_id}:{domain_id}",
                            "source_id": item.object_id,
                            "relation": "belongs_to_domain",
                            "target_id": f"domain:{domain_id}",
                            "confidence": float(scope_payload.get("confidence", 0.0)),
                            "rationale": "来自已激活知识的适用领域。",
                            "evidence_knowledge_ids": [item.object_id],
                        }
                    )
            nodes.extend(
                {
                ...
                nodes.append(
                    {
                        "node_id": cluster_id,
                        "node_type": "cluster",
                        "title": str(cluster.get("name", "")),
                        "summary": str(cluster.get("summary", "")),
                        "confidence": float(cluster.get("confidence", 0.0)),
                        "domain_hypotheses": list(
                            cluster.get("domain_hypotheses", [])
                        ),
                    }
                )
                for member in cluster.get("member_knowledge_ids", []):
                    edges.append(
                        {
                            "edge_id": f"cluster:{member}:{cluster_id}",
                            "source_id": str(member),
                            "relation": "member_of_cluster",
                            "target_id": cluster_id,
                            "confidence": float(cluster.get("confidence", 0.0)),
                            "rationale": "来自一次受控聚类运行。",
                            "evidence_knowledge_ids": [str(member)],
                        }
                    )
            for gap in gaps:
                nodes.append(
                    {
                        "node_id": gap.object_id,
                        "node_type": "knowledge_gap",
                        "title": str(gap.payload.get("question", "")),
                        "priority": float(gap.payload.get("confidence", 0.0)),
                    }
                )
                for knowledge_id in gap.payload.get("related_knowledge_ids", []):
                    edges.append(
                        {
                            "edge_id": f"gap:{knowledge_id}:{gap.object_id}",
                            "source_id": str(knowledge_id),
                            "relation": "has_open_gap",
                            "target_id": gap.object_id,
                            "confidence": float(gap.payload.get("confidence", 0.0)),
                            "rationale": str(
                                gap.payload.get("reason_unresolved", "")
                            ),
                            "evidence_knowledge_ids": [str(knowledge_id)],
                        }
                    )
            return _public(
                {
                    "nodes": nodes,
                    "edges": edges[: max(1, min(limit, 5000))],
                    "clusters": snapshot.get("clusters", []),
                    "explorations": snapshot.get("explorations", []),
                }
            )
